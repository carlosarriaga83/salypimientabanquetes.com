				...
				if (true ) {
					// Append each value wrapped in <td> tags to the current row
						...
						switch ($col) {
							...
							case "DISH_PIC":
								...
								$htmlOutput .= <<<NAME
														...
														<div class="d-flex align-items-center">
													        <img src="{$row['DISH_PIC']}" alt="assets/images/user-list/user-list2.png" class="w-60-px h-60-px rounded-circle flex-shrink-0 me-12 overflow-hidden" onerror="this.onerror=null;this.src='assets/images/user-list/user-list2.png';">
							...
							case "OPT_VEGETARIANO":
									//$row[$col] == 'on' ? $row[$col] = 'si' : $row[$col] = 'no';
									$row[$col] == 'on' ? $ICONO = '<span class="btn-lilac-100 bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle"><iconify-icon icon="hugeicons:vegetarian-food" class="menu-icon"></iconify-icon></span>' : $ICONO = '';
									
									$htmlOutput .= '<td>' . '' . $ICONO . '' . '</td>' . "\n";
								break;

							case "OPT_VEGANO":
									//$row[$col] == 'on' ? $row[$col] = 'si' : $row[$col] = 'no';
									$row[$col] == 'on' ? $ICONO = '<span class="btn-lilac-100 bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle"><iconify-icon icon="lucide:vegan" class="menu-icon"></iconify-icon></span>' : $ICONO = '';
									$htmlOutput .= '<td>' . '' . $ICONO . '' . '</td>' . "\n";
								break;
								...
								$htmlOutput .= <<<H
												<td class="text-center">
													<div class="d-flex align-items-center gap-10 justify-content-center">
														<button type="button" class="bg-info-focus bg-hover-info-200 text-info-600 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle d-none" data-bs-toggle="modal" data-bs-target="#Modal_1" onclick="LOAD_ID({$row['ID']});">
															<iconify-icon icon="majesticons:eye-line" class="icon text-xl"></iconify-icon>
														</button>
														<a  href="{$T_PARAMS['TABLE']['href']}?id={$row['ID']}">
														<button type="button" class="bg-success-focus text-success-600 bg-hover-success-200 fw-medium w-40-px h-40-px d-flex justify-content-center align-items-center rounded-circle edit admin_edit">
